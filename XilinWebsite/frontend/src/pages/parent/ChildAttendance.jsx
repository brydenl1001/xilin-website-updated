...
export default function ParentChildAttendance() {
  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <PageHeader
        title="Attendance"
        subtitle={child ? `${child.name} · ${child.grade}` : 'Attendance record'}
      />
        ...
        <div className="flex gap-3">
          {WEEK.map(({ day, present }) => (
            <div key={day} className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium ${present ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-500'}`}>
                {present ? '✓' : '✗'}
              </div>
            </div>
          ))}
