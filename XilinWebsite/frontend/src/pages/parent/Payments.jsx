...
export default function ParentPayments() {
  const [payments, setPayments] = useState(mockPayments.slice(0, 3))
  ...
  const confirm = (id) => {
    ...
    setTimeout(() => {
      setPayments(prev => prev.map(p => p.id === id
        ? { ...p, status: 'paid', date: new Date().toISOString().slice(0, 10), ref: `SAND-${Math.random().toString(36).slice(2, 8).toUpperCase()}` }
        : p))
      setProcessing(false)
      setPayTarget(null)
    }, 1800)
  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <PageHeader title="Payments" subtitle="Fee history for James Doe" />
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <Card>
          ...
          {pending > 0 && <p className="text-xs text-amber-500 mt-1">Due Jun 1, 2026</p>}
        </Card>
      </div>
          ...
          <Card key={p.id}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{p.date ? `Paid ${p.date}` : 'Not yet paid'}</p>
                {p.ref && <p className="text-xs font-mono text-slate-300 mt-0.5">{p.ref}</p>}
              </div>
              <div className="text-right flex flex-col items-end gap-2">
                <p className="font-display text-lg text-slate-900">${p.amount.toLocaleString()}</p>
        ...
        {payTarget && (
          <>
            ...
            <div className="bg-slate-900 rounded-xl p-4 mb-4">
              ...
              <p className="font-display text-2xl text-yellow-400">${payTarget.amount.toLocaleString()}</p>
              <p className="text-slate-400 text-xs mt-1">{payTarget.description}</p>
            </div>
            <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button variant="gold" onClick={() => confirm(payTarget.id)} disabled={processing}>
                {processing
                  ...
                  : <span className="flex items-center gap-2"><CreditCard size={13} />Pay ${payTarget.amount.toLocaleString()}</span>
                }
              </Button>
            </div>
          </>
