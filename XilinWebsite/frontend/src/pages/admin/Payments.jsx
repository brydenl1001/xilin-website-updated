      ...
      <div className="flex justify-end gap-2 mt-5">
        ...
        <Button variant="gold" onClick={handlePay} disabled={processing}>
          {processing
            ...
            : <span className="flex items-center gap-2"><CreditCard size={13} />Pay ${Number(payment.amount).toLocaleString()}</span>
          }
        </Button>
      </div>
    </Modal>
          ...
          <Table headers={['Student', 'Description', 'Amount', 'Status', 'Date', 'Ref', 'Action']}>
            {filtered.map(p => (
              <Tr key={p.id}>
                ...
                <Td><span className="font-semibold text-slate-900">${Number(p.amount).toLocaleString()}</span></Td>
                <Td><Badge variant={STATUS_VARIANT[p.status]}>{p.status}</Badge></Td>
                <Td className="text-slate-400 text-xs">{p.paid_at?.slice(0, 10) || '—'}</Td>
                <Td className="font-mono text-xs text-slate-400">{p.transaction_ref || '—'}</Td>
                <Td>
