...
export default function StudentGrades() {
  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <PageHeader title="My Grades" subtitle="Academic performance" />
      ...
      <div className="bg-slate-900 rounded-2xl p-6 mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Across {grades.length} recorded grade{grades.length !== 1 ? 's' : ''}</p>
        </div>
        {pcts.length > 0 && (
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Highest', value: `${Math.max(...pcts)}%` },
              { label: 'Lowest', value: `${Math.min(...pcts)}%` },
            ].map(({ label, value }) => (
            ...
            <Card key={g.id} className="animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${pct >= 90 ? 'bg-emerald-400' : pct >= 75 ? 'bg-blue-400' : 'bg-amber-400'}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
