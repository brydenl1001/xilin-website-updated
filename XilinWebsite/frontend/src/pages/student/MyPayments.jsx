...
const MY_PAYMENTS = mockPayments.filter((_, i) => i < 3)
...
export default function StudentMyPayments() {
  ...
  const confirm = (id) => {
    setProcessing(true)
    setTimeout(() => {
      setPayments(prev => prev.map(p => p.id === id ? { ...p, status: 'paid', date: new Date().toISOString().slice(0, 10), ref: `SAND-${Math.random().toString(36).slice(2, 8).toUpperCase()}` } : p))
      setProcessing(false)
      setPayTarget(null)
    }, 1800)
  }
        ...
        {payTarget && (
          <>
            ...
            <div className="bg-slate-900 rounded-xl p-5 mb-5">
              ...
              <p className="font-display text-3xl text-yellow-400">${payTarget.amount.toLocaleString()}</p>
