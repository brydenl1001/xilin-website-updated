...
const SUBJECT_COLORS = ['bg-teal-100 text-teal-700', 'bg-cyan-100 text-cyan-700', 'bg-sky-100 text-sky-700', 'bg-slate-100 text-slate-700', 'bg-emerald-100 text-emerald-700', 'bg-blue-100 text-blue-700']
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="mb-10 text-center">
        <p className="text-teal-600 text-xs uppercase tracking-widest font-medium mb-3">Academics</p>
            ...
            <div key={subject}>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {bySubject[subject].map((course, i) => (
                  <div key={course.id} className="border border-slate-200 bg-white rounded-2xl p-5 hover:border-teal-300 transition-colors">
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      {course.price != null && (
                        <span className="font-medium text-teal-700">${Number(course.price).toLocaleString()}/term</span>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {/* Info grid */}
      <div className="bg-slate-800 rounded-2xl p-8 text-white grid md:grid-cols-3 gap-8">
        {[
          ...
          <div key={title}>
            <div className="w-9 h-9 rounded-lg bg-teal-400/15 flex items-center justify-center mb-3">
